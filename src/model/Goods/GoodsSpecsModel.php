<?php
	...
	class GoodsSpecsModel extends \dackou\Model{
		protected $table = 'GoodsSpecs';
		protected $title = '商品规格参数';
		...
		protected function validate(Request $request,$id = 0,$obj = null){
			...
			if($field_type == 1){
				...
				if(!is_numeric($field_length) || $field_length < 1){
					return '字段长度只能填数字';
				}
			}
		...
		protected function getActionList(Request $request,$id = 0): array
		{
			...
			$action = [
				['type'=>'input','label'=>'规格名称','prop'=>'title','value'=>$id ? $data->title : '','rules'=>['required'=>true,'message'=>'规格名称不能为空']],
		...
		protected function getMapList(Request $request): array
		{
			return [
				...
				['type'=>'varchar','label'=>'规格名称','prop'=>'title'],
